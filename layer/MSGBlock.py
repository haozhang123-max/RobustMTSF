...
class GraphBlock(nn.Module):
    def __init__(self, c_out , d_model , conv_channel, skip_channel,
        ...
        # self.gconv2 = GCN(conv_channel, skip_channel, gcn_depth, dropout)
        self.gelu = nn.GELU()
    ...
    def forward(self, x,k):
        ...
        # # 将邻接矩阵转换为 NumPy 数组
        # # 将邻接矩阵转换为 NumPy 数组
        # adj_matrix_numpy = adp.detach().cpu().numpy()
        #
        # # 绘制热力图
        # plt.figure(figsize=(8, 6))  # 设置图像大小
        # plt.imshow(adj_matrix_numpy, cmap='Reds', interpolation='nearest')  # 使用红色渐变颜色映射
        # plt.colorbar()  # 显示颜色条
        # plt.title("scale"+str(k))  # 设置标题
        #
        # # 添加刻度标签（可选）
        # plt.xticks(np.arange(adj_matrix_numpy.shape[1]))
        # plt.yticks(np.arange(adj_matrix_numpy.shape[0]))
        # plt.savefig('hot' + str(k) + '.png', facecolor='white')  # 保存为 PNG 格式

        # adp = F.softmax(F.relu(torch.mm(self.nodevec1, self.nodevec2)), dim=1)
        # threshold = 0.5
        # adp = torch.where(adp > threshold, adp, torch.tensor(0.0))
