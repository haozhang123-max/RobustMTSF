...
from layer.MSGBlock import GraphBlock, simpleVIT, Attention_Block, Predict
...
class ScaleGraphBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        #for Inception Block
            ...
            # out = out.reshape(B, length // scale, scale, N).reshape(B, -1, N)
        ...
        res = torch.stack(res,
                          dim=-1)  # 假设 query、key 和 value 的形状与 res 相同
...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        # 点积计算邻接矩阵
        # nodevec1 = self.layer_norm1(x_enc).reshape(-1,N).permute(1,0)
        # nodevec2 = self.layer_norm1(x_enc).reshape(-1,N)
        #
        ...
        # 余弦相似度计算邻接矩阵
        nodevec1 = self.layer_norm1(x_enc).reshape(-1, N).permute(1,0)
        nodevec2 = self.layer_norm1(x_enc).reshape(-1, N).permute(1,0)
        ...
        # dec_out = dec_out*0.01  + dec_out1
        # dec_out = dec_trend + dec_seasonal
